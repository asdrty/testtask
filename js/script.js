...
function myMassange(userGend) {
  let mass =
    '<div class="chat-content-item user "><div class="chat-content-desc"><div class="chat-content-desc-item user"><p class="textUser">' +
    userGend +
    "</p></div></div></div>";
...
// Прокрутка вниз по натиску кнопки

const btn_scroll_me = document.getElementById("scroll_id");

btn_scroll_me.addEventListener("click", scrollDown);

// Коментарі

const glow_button = document.querySelector(".glow-button");
const allComment = document.querySelectorAll('.comment-item');
const reviews_title = document.querySelector(".reviews-title")
let indexComment = 3;

function showComent() {
  if (indexComment<allComment.length) {
  allComment[indexComment++].classList.add('comment-item-show');
  scrollDown() } else {
    glow_button.classList.remove('glow-button')
    glow_button.classList.add("glow-button--hide");
    glow_button.removeEventListener('click', showComent)
  }
}

function titleShowComment() {
  if (allComment[1].classList.contains("comment-item-show")) {
    for (let i=0; i<allComment.length; i++) {
      allComment[i].classList.remove("comment-item-show")
    }
    glow_button.style.display = 'none'
  } else {
    for (let i=0; i<3; i++) {
      allComment[i].classList.add("comment-item-show")
    }
    glow_button.style.display = 'block'
  }
}

reviews_title.addEventListener("click", titleShowComment);
glow_button.addEventListener("click", showComent);

// валідація форми 

const input_name = document.getElementById("input-name");
const input_phone = document.getElementById("input-phone");
const btn_submit = document.querySelector(".submit-roulette");
let regex = /^[\+][\d\(\)\ -]{11,14}\d$/; 
let nameIs = false;
let phoneIs = false;
...
function checkPhone() {
  let phon = input_phone.value
  input_phone.value = input_phone.value.replace (/[^\+\d]/g, '');
  if ((!regex.test(phon))||(!(phon[1]==="3"))||(!(phon[2]==="8"))) {
    input_phone.classList.add("hide_form");
    phoneIs = false;
    }else{
    input_phone.classList.remove("hide_form");
    phoneIs = true;
    }
  checkForm()
}
...
function checkName() {
  if(input_name.value.length<2) {
    input_name.classList.add("hide_form");
    phoneIs = false;
  } else {
    input_name.classList.remove("hide_form");
    nameIs = true;
  }
  checkForm()
}

input_phone.addEventListener('input', checkPhone);
input_name.addEventListener("input", checkName)

// відправка форми 

async function postForm () {
  const getAnswer = document.querySelectorAll('.textUser');
  const bodyForm = {
    name: input_name.value,
    tel: input_phone.value,
    gender: getAnswer[0].textContent,
    birthday: getAnswer[1].textContent,
    allergy: getAnswer[2].textContent
  }
...
btn_submit.addEventListener("click", postForm)